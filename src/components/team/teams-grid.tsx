...
import { teams } from '@/lib/mayfest-teams/teams';
...
export default function TeamsGrid() {
  ...
  if (open !== null) {
    const insertIndex = isMobile ? Math.ceil(open / 2) * 2 : Math.ceil(open / 5) * 5;
    teamBoxes.splice(insertIndex, 0, (
      <TeamInfo
        key="team-info"
        team={teams[open]}
  ...
  return (
    <motion.div
      className="grid w-full grid-cols-5 gap-4 mx-auto text-sm sm:grid-cols-2"
      variants={container}
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true }}
